...
//    }
    ...
    def "should validate that all YAML queries are accessible"() {
        given:
        def expectedQueries = [
            "UserMapper.findUserById",
            "UserMapper.findActiveUsers",
            "UserMapper.findUsersWithDynamicFilters",
            "GuiaMapper.findGuiaById",
            "GuiaMapper.findGuiasByNumero"
        ]
        
        expect:
        expectedQueries.each { queryName ->
            def users = userRepository.executeNamedQuery(queryName, [:])
            assert users != null
        }
    }
